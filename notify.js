...
$.notify.addStyle('foo', {
	html: 
		"<div>" +
			...
			"<div class='buttons'>" +
				"<button class='no' data-notify-text='cancel'></button>" +
				"<span class='id' data-notify-html='id'></span>" +
...
$(document).on('click', '.notifyjs-foo-base .yes', function() {
	...
	$.ajax({
		url: elk_scripturl + '?action=dismissnotice;idnotice=' + id + ';' + elk_session_var + '=' + elk_session_id + ';api;xml'
	})
	.always(function() {
		//programmatically trigger propogating hide event
		$this.trigger('notify-hide');
...
function dismissnotice_editable()
{
	$(document).on('click', '.dismissnotice_editable', function(e) {
		e.preventDefault();

		var $modify = $(this);

		if ($modify.data('state') == 'open')
			return;

		$modify.data('state', 'open');

		var id = $modify.data('idnotice'),
			$row = $modify.closest('.editable_' + id);

		$.ajax({
			url: elk_scripturl + '?action=admin;area=news;sa=notices;edit;idnotice=' + id + ';' + elk_session_var + '=' + elk_session_id + ';api;xml',
			dataType: 'html',
			beforeSend: ajax_indicator(true)
		})
		.done(function(request) {
			create_popup($row, id, request, function(request) {
				var row_n = $row.parent().children().index($row),
					$new_r = create_new_row(row_n, request);
	...
	$('#dismissnotice_new').on('click', function(e) {
		...
		.done(function(request) {
			create_popup($row, 0, request, function(request) {
				var $new_r = create_new_row($('#list_dismissible_notices tr').length - 2, request);
	...
	function create_new_row(num_rows, request)
	{
		var $new_r = $('<tr class="standard_row editable_' + request.id + '" id="list_list_dismissible_notices_' +  num_rows + '" />');
